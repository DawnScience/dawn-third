...
import org.dawnsci.usagedata.internal.recording.filtering.FilterChangeListener;
import org.dawnsci.usagedata.internal.recording.filtering.FilterUtils;
...
import org.dawnsci.usagedata.internal.recording.uploading.UsageDataFileReader;
import org.dawnsci.usagedata.internal.ui.Activator;
...
public class UploadPreview  {
	...
	public Control createControl(final Composite parent) {
		...
		createDAWNOnlyButton(composite);
	...
	private void createDAWNOnlyButton(Composite buttons) {
		dawnOnlyButton = new Button(buttons, SWT.CHECK);
		dawnOnlyButton.setText(Messages.UploadPreview_12); 
		dawnOnlyButton.setSelection(true);
		dawnOnlyButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				((PreferencesBasedFilter)parameters.getFilter()).setNonBundleEventOnly(dawnOnlyButton.getSelection());
			}
		});
	}
	...
	private void updateButtons() {
		if (parameters.getFilter() instanceof PreferencesBasedFilter) {
			...
			if (filter.isNonBundleEventOnly()) {
				dawnOnlyButton.setSelection(true);
				addFilterButton.setEnabled(false);
				removeFilterButton.setEnabled(false);
				eclipseOnlyButton.setEnabled(false);
			} else {
				dawnOnlyButton.setSelection(false);
				addFilterButton.setEnabled(true);
				removeFilterButton.setEnabled(filter.getFilterPatterns().length > 0);
				eclipseOnlyButton.setEnabled(true);
			}
		}
	}
