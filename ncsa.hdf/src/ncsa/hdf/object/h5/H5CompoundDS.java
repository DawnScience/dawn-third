...
public class H5CompoundDS extends CompoundDS {
    ...
    public void init() {
        ...
        long did = -1, sid = -1, tid = -1, tclass = -1;
        ...
        if (did >= 0) {
            ...
            try {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    member_name = new String(memberNames[i]);
                    atom_tid = atomicList.get(i).longValue();
    ...
    public static Dataset create(String name, Group pgroup, long[] dims, long[] maxdims, long[] chunks, int gzip,
        ...
        long did = -1, sid = -1, tid = -1, plist = -1;
     ...
     * </pre>
     */
    private void extractCompoundInfo(long tid, String name, List<String> names, List<Long> types) {
     ...
     * (non-Javadoc)
     * 
     * @see hdf.object.Dataset#isString(int)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see hdf.object.Dataset#getSize(int)
     */
    @Override
     ...
     */
    private final long createCompoundFieldType(long member_tid, String member_name, int[] compInfo) throws HDF5Exception {
    	long nested_tid = -1;
